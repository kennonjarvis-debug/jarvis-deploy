-- =============================================================================
-- WEEK 2: COMBINED MIGRATION FILE
-- =============================================================================
-- This file combines all Week 2 migrations for easy application via Supabase Dashboard
-- Copy and paste this entire file into Supabase Dashboard → SQL Editor → New Query
-- Then click "Run" to apply all migrations at once
--
-- Includes:
-- 1. Add onboarding fields to observatories table
-- 2. Create subscriptions table for Stripe integration
-- 3. Create connections tables for integration management
-- =============================================================================

-- =============================================================================
-- MIGRATION 1: Add Onboarding to Observatories
-- Date: 2025-01-20
-- =============================================================================
...
COMMENT ON COLUMN observatories.onboarding_data IS 'Stores business profile data from onboarding: name, industry, brand voice, etc.';
...
-- =============================================================================
-- MIGRATION 3: Create Connections Tables
-- Date: 2025-01-20
-- =============================================================================
...
DO $$
BEGIN
  -- Check if all new columns/tables exist
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'observatories'
    ...
    SELECT 1 FROM information_schema.tables
    WHERE table_name = 'connection_settings'
  ) AND EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_name = 'business_info_extractions'
  ) THEN
    RAISE NOTICE '✅ Week 2 migrations applied successfully!';
    RAISE NOTICE 'Created/Updated:';
    RAISE NOTICE '  - observatories.onboarding_completed';
    RAISE NOTICE '  - observatories.onboarding_data';
    RAISE NOTICE '  - subscriptions table';
    RAISE NOTICE '  - connections table';
    RAISE NOTICE '  - connection_settings table';
    RAISE NOTICE '  - business_info_extractions table';
    RAISE NOTICE '  - All RLS policies';
    RAISE NOTICE '  - All indexes and views';
  ELSE
    RAISE WARNING '⚠️ Some migrations may not have completed. Please check the error messages above.';
  END IF;
END $$;
